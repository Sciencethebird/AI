...
eta =  0.00001      #learning rate       ###########lolololololol
...
for itr in range(iteration):
    ...
    # Bakcward-propagation
        ...
        dEdx, dEdW, dEdb = InnerProduct_Back( ReLu_Back(dEdx , z1), train_data_img, w_1, b_1 )
        b1_grad = dEdb 
        w1_grad = dEdW
    ...
    # Get 1-D Prediction array
        ...
        zout = ReLu_For( InnerProduct_For(valid_data_img, w_1, b_1) )
        #Softmax
        yp = Softmax_For( InnerProduct_For(zout, w_out, b_out) ) 
    ...
    # Calculate the accuracy
        ...
        if itr > 70 and  eta > 0.0000005 and accuracy - valid_accuracy[-1] < -0.15 :
            eta = eta / 1.1
            print('eta changed down to ', eta)
        if accuracy - valid_accuracy[-1]< 0.07 and accuracy - valid_accuracy[-1] > 0 and itr>300:
            eta = eta * 1.1
...
# Forward-propagation
# In[10]:
zout = ReLu_For( InnerProduct_For(test_data_img, w_1, b_1) )
...
df.to_csv("DL_LAB1_prediction_Softmax_ReLU.csv",index=True, index_label="index")
